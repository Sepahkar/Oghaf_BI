    ...
    for p in PROVINCES:
        ...
        if p_name in ['فارس', 'مازندران']:
            ...
            for c in real_counties:
                ...
                nosand_c = c['no_doc'] * random.uniform(1.5, 3.5) # Estimate properties without doc
                nosand_c = int(nosand_c)
            ...
            # Distribute endowments randomly among counties
        ...
        # Insert counties and sample endowments
        for c_idx, c_data in enumerate(counties_data):
            ...
            # Area estimation (randomly scaled from count)
            ...
            # Generate sample endowments for drill-down (only max 10 to keep db generation fast)
            sample_size = min(10, (c_data['takbarg_c'] + c_data['daftarchei_c'] + c_data['nosand_c']) // 5 + 1)
            sample_size = min(sample_size, 15)
...
if __name__ == '__main__':
    ...
    conn.close()
    print("Database Generated Successfully with Scaled Data.")
